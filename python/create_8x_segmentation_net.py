...
def main(saved_16x_weights):
	npz = np.load(saved_16x_weights)
	...
	# Upsample 16x prediction layer to 8x layer
	upsampling_16_to_8_W = tf.Variable(bilinear.create_initial_bilinear_weights(4), name='upsampling_16_to_8_W')
	upsampling_16_to_8_b = tf.Variable(tf.truncated_normal([21], stddev=0.1), name='upsampling_16_to_8_b')

	upsampled_from_16x = tf.add(tf.nn.conv2d_transpose(downsampled_16x, upsampling_16_to_8_W, [BATCH_SIZE, stride_8(image_height), stride_8(image_width), 21], [1, 2, 2, 1]), upsampling_16_to_8_b, name='upsampled_from_16x')

	# Create coarse predictions from pool3 layer
	pool3_predictions_W = tf.Variable(tf.zeros([1, 1, 256, 21]), name='pool3_predictions_W')
	pool3_predictions_b = tf.Variable(tf.zeros([21]), name='pool3_predictions_b')

	pool3_predictions = tf.add(tf.nn.conv2d(pool3, pool3_predictions_W, [1, 1, 1, 1], "SAME"), pool3_predictions_b, name='pool3_predictions')

	# Combine both 8x predictions
	downsampled_8x = tf.add(upsampled_from_16x, pool3_predictions)

	# Upsample 8x
	upsampling_8x_W = tf.Variable(bilinear.create_initial_bilinear_weights(16), name='upsampling_8x_W')
	upsampling_8x_b = tf.Variable(tf.truncated_normal([21], stddev=0.1), name='upsampling_8x_b')

	upsampled_8x_output = tf.add(tf.nn.conv2d_transpose(downsampled_8x, upsampling_8x_W, [BATCH_SIZE, image_height, image_width, 21], [1, 8, 8, 1]), upsampling_8x_b, name='upsampled_8x_output')
	...
	loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=upsampled_8x_output, labels=ground_truth_without_void), name='loss')
	train = tf.train.AdamOptimizer(LEARNING_RATE * 0.01).minimize(loss, name='train')

	predictions = tf.argmax(upsampled_8x_output, axis=3, name='predictions')
	...
	saver.save(sess, 'initial_model/initial_8x_model')
...
def stride_16(n):
	stride_16 = (stride_8(n) + 1) / 2
	return stride_16

if __name__ == '__main__':
	parser = argparse.ArgumentParser()
	parser.add_argument('saved_16x_weights', help='Path to saved 16x weights')
	args = parser.parse_args()
	main(args.saved_16x_weights)