...
def main(weights_file):
	...
	# Layer 6
	conv6_W = tf.Variable(tf.truncated_normal([1, 1, 512, FULLY_CONV_DEPTH], stddev=0.1), name='conv6_W')
	conv6_b = tf.Variable(tf.truncated_normal([FULLY_CONV_DEPTH], stddev=0.1), name='conv6_b')
	...
	# Layer 7
	conv7_W = tf.Variable(tf.truncated_normal([1, 1, FULLY_CONV_DEPTH, FULLY_CONV_DEPTH], stddev=0.1), name='conv7_W')
	conv7_b = tf.Variable(tf.truncated_normal([FULLY_CONV_DEPTH], stddev=0.1), name='conv7_b')
	...
	downsampled_32x_W = tf.Variable(tf.zeros([1, 1, FULLY_CONV_DEPTH, 21]), name='downsampled_32x_W')
	downsampled_32x_b = tf.Variable(tf.zeros([21]), name='downsampled_32x_b')

	downsampled_32x = tf.add(tf.nn.conv2d(drop7, downsampled_32x_W, [1, 1, 1, 1], "SAME"), downsampled_32x_b, name='downsampled_32x')

	# Upsample 32x
	upsampling_32x_W = tf.Variable(bilinear.create_initial_bilinear_weights(64), name='upsampling_32x_W')
	upsampling_32x_b = tf.Variable(tf.truncated_normal([21], stddev=0.1), name='upsampling_32x_b')

	upsampled_32x_output = tf.add(tf.nn.conv2d_transpose(downsampled_32x, upsampling_32x_W, [BATCH_SIZE, image_height, image_width, 21], [1, 32, 32, 1]), upsampling_32x_b, name='upsampled_32x_output')
	...
	loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=upsampled_32x_output, labels=ground_truth_without_void), name='loss')
